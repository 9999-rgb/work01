...
class SensorFailureContractTest(unittest.TestCase):
    ...
    def test_standalone_camera_endpoint_rejects_a_stale_last_frame(self) -> None:
        from sensor_bridge.state import SensorStreamState
        from sensor_bridge.web_server import SensorWebHandlers
        ...
        with patch("sensor_bridge.state.time.monotonic", return_value=12.0):
            response = asyncio.run(handlers.camera_jpeg(MagicMock()))
        self.assertEqual(response.status, 503)

    def test_standalone_lidar_websocket_closes_when_scan_is_stale(self) -> None:
        from sensor_bridge.state import SensorStreamState
        from sensor_bridge.web_server import SensorWebHandlers
        ...
        async def mark_closed(*_args, **_kwargs) -> None:
            websocket.closed = True

        websocket.close = AsyncMock(side_effect=mark_closed)
        request = MagicMock()
        with (
            patch(
                "sensor_bridge.web_server.web.WebSocketResponse",
                return_value=websocket,
            ),
            patch("sensor_bridge.state.time.monotonic", return_value=12.0),
        ):
            returned = asyncio.run(handlers.lidar_websocket(request))

        self.assertIs(returned, websocket)
