...
末端工具是「操作柜子按钮」的枢纽件,与柜体耦合最紧:

- `contact_tool_link` —— 按按钮 / 旋转旋钮 / 摇入摇出的接触点长在固定末端工具上(如 `r_three_cyl_base` / `r_rotbtn_body` / `l_rocker_body`);
- `unreachable_control_ids` / `navigation_station` —— 「够不够得到某控件、从哪接近」取决于工具几何与挂装臂;
- 旋钮 / 开关 / 门的抓取(`SetCabinetGrasp`)依赖对应臂的固定工具。

因此「换柜体」会触碰**工具 × 柜体可达性**。工具随机器人固定(右手:三电缸 +
旋转按钮,左手:两电缸 + 摇入摇出),故每导入一台新柜体只需补一份对该柜体的
可达性评价(`unreachable_control_ids` 清单,或后续用现有 `operable=false`
规划验证自动推导)。
...
(`jiang/data/assets/`)→ 登记 catalog(SQLite `assets` 表)。

### 5.2 选择(Select)
Web 选择场景 / 柜体 → 持久化到 SQLite(`selection` 表,与 auth 同库)→
...
SQLAlchemy 栈与 `XCZS_DATABASE_URL`)。`AssetLibrary` 改为 store 注入
(`AssetStore` 协议),`YamlAssetStore` 保留为参考 / 纯库单测实现,
`app.assets.store.SqlAssetStore` 为生产实现(同步 `Session`,Web 经
...
- Web 闭环(TestClient 驱动真实 create_app + 真实资产库):
  ...
  - 选择校验:未知场景 → 404、空串清除选择;SQLite `selection` 表落盘;
...
- **工具 × 柜体可达性**:新柜体需预写可达性表(`unreachable_control_ids`);后续可复用
...
- **资产版本与兼容**:manifest 带 `version`,导入校验可回答「这柜体与当前机器人配不配」。
