...
后门采用过中心双档机构。机械臂只执行初始位置到目标位置行程的 53%，即开门带到约 47.7°、关门带回约 42.3°后释放。门档仅在近距物理约束已建立时按 0.02 rad 迟滞阈值切换；机械臂引导期间阻尼为 2.0、档位预载刚度为 0.5，释放后锁存目标档位，再切换到 27.0 的阻尼和 20.0 的刚度，由接近门板实际转动惯量临界阻尼的物理回弹完成到 0°或 90°。系统最长等待 90 s，并以释放后的新状态样本和低速度状态验收。
