...
三个装饰把手部件不参与 Gazebo 接触，门板实体碰撞仍保留，并与固定柜框的四周及深度方向各留出 10 mm 装配间隙。柜门夹持由经过近距校验的固定约束承担，避免零间隙柜框摩擦或释放约束后探针与装饰把手二次碰撞，将大质量门板卡住或打过锁止档位。
...
后门采用过中心双档机构。机械臂只执行初始位置到目标位置行程的 53%，即开门带到约 47.7°、关门带回约 42.3°后释放。门档仅在近距物理约束已建立时按 0.02 rad 迟滞阈值切换；机械臂引导期间阻尼为 2.0，释放后锁存目标档位，再切换到接近门板临界阻尼的 12.0，并由刚度 5.0 的物理回弹完成到 0°或 90°。系统最长等待 45 s，并以释放后的新状态样本和低速度状态验收。
