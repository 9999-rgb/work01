...
- 关节控制：手动关节指令（`/xczs/joint_trajectory`）由 `legacy_trajectory_router` 按 `manual_joint_group_names` 逐组拆到 `/xczs/<组名>_controller/joint_trajectory`（本工程为 left_arm / right_arm / three_cylinder / two_cylinder / rocker / rotate_button 六组）。早期的 `/xczs/arm_controller`、`/xczs/gripper_controller` 属兼容分支，仅在该参数缺失时生效，本适配层不会走。全栈控制入口是 Web（`jiang/`），控制包不再提供本地键盘/GUI 遥控节点。
...
- 操作互斥：`operation_lease_coordinator` 通过 `/xczs/operation_lease` 服务（`ManageOperationLease.srv`）分配租约。该节点自身没有就绪输入（构造即建服务并授租），启动时序门在别处：launch 把 router/moveit/nav2 挡在位姿校验之后，operator 侧再 `wait_for_service`。租约边界规则在 `operation_lease_state.hpp`；`operation_validation_policy.hpp` 讲的是基座运动准备与物理运动资源所有权。
