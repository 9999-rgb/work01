...
输出柜体盒的有符号距离、工具/按钮板双向顶点穿入深度和双钳口到轴表面的
距离。顶点采样不是完整连续碰撞证明；应与 MoveIt 碰撞检查及 Gazebo 接触合看。
"""
import argparse
import json
    ...
    while time.monotonic() - started < args.duration:
        ...
        for name, points in world.items():
            ...
            for fixture, fp in world.items():
                ...
                if fixture.endswith('1') and low < plate_min[0]:
                    plate_min = [low, name, fixture]
                if fixture.endswith('1') and 'jaw' in name:
                    shaft_dist[name + ':' + fixture] = low
        print(json.dumps({'wall_time': time.time(), 'elapsed': last-started, 'joints': joints, 'link_poses': {k: p.tolist() for k, p in poses.items() if 'rotbtn' in k or k.startswith('xczs_scene_floor::')}, 'cabinet_signed_m': box_min, 'button_signed_m': plate_min, 'jaw_plate_signed_m': shaft_dist}), flush=True)
    node.destroy_node()
    rclpy.shutdown()
